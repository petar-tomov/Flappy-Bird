...
const bird = {
	...
	update : function(){
		// IF THE GAME STATE IS GET READY, BIRD FLOPS SLOWER
		...
		if (state.current == state.getReady){
			this.y = 150; // RESET THE POSITION AFTER THE GAME'S OVER
			...
			// IF THE SPEED IS GREATER THAN THE JUMP => BIRD IS FALLING DOWN
...
const pipes = {
	...
	update : function(){
		...
		for(let i = 0; i < this.position.length; i++){
			...
			// if the pipes go beyond the canvas
			if (p.x + this.w <= 0){
				this.position.shift();
				score.value += 1;
				SCORE_S.play();
